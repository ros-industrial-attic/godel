...
{
public:
  PolygonOffset():  verbose_(false), init_ok_(false),
                    offset_(0.), initial_offset_(0.), discretization_(std::numeric_limits<double>::max())
  {};
   ...
   * Order of polygons is convenient for godel sanding. Smallest path grows outwards, then jumps to next smallest path.
...
private:
  ...
  double discretization_;               /**<Max linear or arc-length distance between adjacent points in resultant. */
