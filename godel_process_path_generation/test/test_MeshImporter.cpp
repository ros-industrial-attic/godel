...
protected:
  ...
  void createMesh(int dir)
  {
     ...
     *  |/ \|
     *  3---4 */
    p0.normal_x = (dir==0)?1:0; p0.normal_y = (dir==1)?1:0; p0.normal_z = (dir==2)?1:0;
    p4=p3=p2=p1=p0;

    if (dir == 0)       //normal in X-direction
    {
      p0.x=0; p0.y=1;  p0.z=1;  pointcloud_->push_back(p0);
      p1.x=0; p1.y=1;  p1.z=-1; pointcloud_->push_back(p1);
      p2.x=0; p2.y=0;  p2.z=0;  pointcloud_->push_back(p2);
      p3.x=0; p3.y=-1; p3.z=1;  pointcloud_->push_back(p3);
      p4.x=0; p4.y=-1; p4.z=-1; pointcloud_->push_back(p4);
    }
    else if (dir == 1)  // normal in Y-direction
    {
      p0.x=-1; p0.y=0; p0.z=1;  pointcloud_->push_back(p0);
      p1.x=-1; p1.y=0; p1.z=-1; pointcloud_->push_back(p1);
      p2.x=0;  p2.y=0; p2.z=0;  pointcloud_->push_back(p2);
      p3.x=1;  p3.y=0; p3.z=1;  pointcloud_->push_back(p3);
      p4.x=1;  p4.y=0; p4.z=-1; pointcloud_->push_back(p4);
    }
    else                // normal in Z-direction
    {
      p0.x=-1; p0.y=1;  p0.z=0; pointcloud_->push_back(p0);
      p1.x=1;  p1.y=1;  p1.z=0; pointcloud_->push_back(p1);
      p2.x=0;  p2.y=0;  p2.z=0; pointcloud_->push_back(p2);
      p3.x=-1; p3.y=-1; p3.z=0; pointcloud_->push_back(p3);
