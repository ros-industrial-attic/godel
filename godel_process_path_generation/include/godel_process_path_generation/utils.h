...
std::vector<Pt> discretizeArc2D(const Pt &p1, const Pt &p2, const Pt &c, bool positive_rotation, double max_sep)
{
  ...
  if (std::abs(theta) > max_theta)
  {
    ...
    Eigen::Vector3d start_pt(p1.x-c.x, p1.y-c.y, 1.);   // Start point expressed in local frame (c in global frame)
 ...
 * @param to Container to move to. If successfully completed, item referenced by "iter" is added to "to".
