...
/** \brief  Header file for all ikfast c++ files/shared objects.
    ...
    - <b>Common</b> - the abstract classes section that all ikfast share regardless of their
   settings
    - <b>Library Specific</b> - the library-specific definitions, which depends on the
   precision/settings that the library was compiled with

    The defines are as follows, they are also used for the ikfast C++ class:

   - IKFAST_HEADER_COMMON - common classes
   - IKFAST_HAS_LIBRARY - if defined, will include library-specific functions. by default this is
   off
   ...
   - IKFAST_ASSERT - Define in order to get a custom assert called when NaNs, divides by zero, and
   other invalid conditions are detected.
   - IKFAST_REAL - Use to force a custom real number type for IkReal.
   - IKFAST_NAMESPACE - Enclose all functions and classes in this namespace, the ``main`` function
   is excluded.
...
template <typename T> class IkSingleDOFSolutionBase
{
public:
  ...
  unsigned char indices[5]; ///< unique index of the solution used to keep track on what part it
                            ///came from. sometimes a solution can be repeated for different
                            ///indices. store at least another repeated root
};
...
/// Stores all these solutions in the form of free variables that the user has to set when querying
/// the solution. Its prototype is:
template <typename T> class IkSolutionBase
{
public:
  ...
  /// \brief Gets the indices of the configuration space that have to be preset before a full
...
template <typename T> class IkSolutionListBase
{
public:
  ...
  /// \param vfree If the solution represents an infinite space, holds free parameters of the
  /// solution that users can freely set.
  ...
  /// \brief clears all current solutions, note that any memory addresses returned from \ref
  /// GetSolution will be invalidated.
...
public:
  IkFastFunctions()
      : _ComputeIk(NULL), _ComputeFk(NULL), _GetNumFreeParameters(NULL), _GetFreeParameters(NULL),
        _GetNumJoints(NULL), _GetIkRealSize(NULL), _GetIkFastVersion(NULL), _GetIkType(NULL),
...
template <typename T> class IkSolution : public IkSolutionBase<T>
{
public:
  ...
  std::vector<IkSingleDOFSolutionBase<T> >
      _vbasesol; ///< solution and their offsets if joints are mimiced
  std::vector<int> _vfree;
};

/// \brief Default implementation of \ref IkSolutionListBase
template <typename T> class IkSolutionList : public IkSolutionListBase<T>
{
public:
  virtual size_t AddSolution(const std::vector<IkSingleDOFSolutionBase<T> >& vinfos,
...
/** \brief Computes all IK solutions given a end effector coordinates and the free joints.
   ...
   - ``eetrans`` - 3 translation values. For iktype **TranslationXYOrientation3D**, the z-axis is
   the orientation.
   - ``eerot``
   ...
   - For **TranslationXAxisAngle4D**, **TranslationYAxisAngle4D**, and **TranslationZAxisAngle4D**
   the first value represents the angle.
   - For **TranslationLocalGlobal6D**, the diagonal elements ([0],[4],[8]) are the local translation
   inside the end effector coordinate system.
 */
...
/// \brief Computes the end effector coordinates given the joint values. This function is used to
/// double check ik.
